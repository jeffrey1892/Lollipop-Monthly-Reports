...
function buildCommentIntelligence(records: ResponseRecord[], priorRecords: ResponseRecord[] = []) {
  ...
  const themeTable = [
    ...
    { theme: 'Optimism / Momentum', trendDirection: countWhere(optimismWords) > 2 ? 'Increasing' as const : 'Stable' as const, sentimentType: 'Positive' as const, primaryTeams: teamList(recordsWhere(optimismWords)), interpretation: 'Commercial momentum and confidence are visible positive drivers in parts of the workforce.', suggestion: 'Name the wins driving momentum in an all-team message; tie them to behaviors others can replicate.' },
  ]
