...
export function getReport(customerId = 'cosmo-cabinets', month?: string): ReportMetrics {
  ...
  const engagementDrop = participationChange !== null && participationChange < -10
  ...
  const recommendations = [
    engagementDrop
      ? { title: 'Rebuild check-in participation', priority: 'P1' as const, urgency: 'High', impact: 'High', difficulty: 'Low', owner: 'HR / operations leader + managers', nextStep: 'Use a short participation push: team-meeting reminders, employee email notices, and a small giveaway/incentive tied to completing check-ins.', why: 'Engagement declined versus the prior month; if fewer employees participate, leaders lose visibility into silent-team risk.', confidence: engagement.confidence, links: [{ label: 'Draft employee reminder email', href: employeeEmail }, { label: 'Manager tools', href: managerToolsUrl }] }
      : { title: 'Maintain check-in participation cadence', priority: 'P2' as const, urgency: 'Medium', impact: 'High', difficulty: 'Low', owner: 'HR / operations leader + managers', nextStep: 'Keep Lollipop visible through team-meeting reminders, periodic email notices, and optional giveaways when participation begins to soften.', why: 'Participation quality protects the usefulness of the report and helps leadership hear from more than the most vocal employees.', confidence: engagement.confidence, links: [{ label: 'Draft employee reminder email', href: employeeEmail }, { label: 'Manager tools', href: managerToolsUrl }] },
    { title: 'Target watch-team intervention', priority: 'P1' as const, urgency: retentionRisk === 'Elevated' || retentionRisk === 'High' ? 'High' : 'Medium', impact: 'High', difficulty: 'Medium', owner: 'Operations leader + HR partner', nextStep: `Hold a 30-minute review with managers for ${watchTeams.slice(0, 2).map((t) => t.team).join(' and ')} to identify workload, communication, and recognition blockers. Ask managers to use the Lollipop manager tools before the discussion.`, why: 'Risk is concentrated by team; targeted intervention is more useful than a broad corporate message.', confidence: 'Medium' as const, links: [{ label: 'Manager tools', href: managerToolsUrl }, { label: 'Draft manager email', href: managerEmail }] },
    { title: 'Operationalize follow-up accountability', priority: 'P2' as const, urgency: 'Medium', impact: 'High', difficulty: 'Medium', owner: 'HR leader + team managers', nextStep: 'Require status, owner, first-response date, and closure note for every follow-up request. Remind managers to use the manager tools to track action items.', why: 'Responsiveness is a leadership-effectiveness signal; missing data prevents Lollipop from proving closure.', confidence: responsiveness.confidence, links: [{ label: 'Manager tools', href: managerToolsUrl }, { label: 'Draft manager email', href: managerEmail }] },
